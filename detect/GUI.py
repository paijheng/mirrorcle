    ...
    def main_loop(self):
        ...
        if self.process.bpms.__len__() >50:
            if(max(self.process.bpms-np.mean(self.process.bpms))<5): #show HR if it is stable -the change is not over 5 bpm- for 3s
                stable_HR = float("{:.2f}".format(np.mean(self.process.bpms)))
                self.lblHR2.setText("Heart rate: " + str(stable_HR) + " bpm")
                now = time.time()
                if(self.ini == True or (now - self.time) >= 10):
                    self.ini = False
                    foo1, foo2, foo3, foo4, detect_status, foo5 = self.process.fd.face_detect(self.process.frame_in)
                    if detect_status == True:
                        sqltask = "INSERT INTO heart_rate(bpm,now) VALUES (%s,%s)"
                        today = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
                        self.time = time.time()
                        data = (stable_HR,today)
                        cursor.execute(sqltask,data)
                        maxdb.commit()
                        print(stable_HR," bpm is saved successfully!")
                    else:
                        print("detecting...")
                    cv2.imwrite("detect.jpg", detect(str(stable_HR), detect_status))
...
def detect(heartrate, status):
    img = np.zeros((1080, 1920, 3), np.uint8)
    now = datetime.now()
    current_time = now.strftime("%H:%M %m/%d %A")
    cv2.putText(img, current_time, (10, 40), cv2.FONT_HERSHEY_DUPLEX, 1, (255, 255, 255), 1, cv2.LINE_AA)
    if status == True:
        cv2.putText(img, "Heart rate: "+heartrate+" bpm", (1500, 540), cv2.FONT_HERSHEY_DUPLEX, 1, (255, 255, 255), 1, cv2.LINE_AA)
    else:
        cv2.putText(img, "Detecting...", (800, 500), cv2.FONT_HERSHEY_DUPLEX, 2, (255, 255, 255), 1, cv2.LINE_AA)
    return img
